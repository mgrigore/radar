...
module.exports = {
  ...
  refresh: function (date) {
    ...
    function primary() {
      let queries = [
        'UPDATE sectii_tara s SET last_update = \'' + date + '\', nr_votanti = _.voturi, pr_votanti = ROUND(COALESCE(100::real * _.voturi / s.votanti_estimati::integer, 0)::numeric, 2) FROM sync _ WHERE _.circumscriptie = s.nce_judet::smallint AND _.sectie = s.numar_sectie::smallint',
        'UPDATE sectii_diaspora s SET last_update = \'' + date + '\', nr_votanti = _.voturi, pr_votanti = ROUND(COALESCE(100::real * _.voturi / NULLIF(s.votanti_estimati::integer, 0), 100::real * _.voturi / NULLIF(_.voturi, 0),  0)::numeric, 2), gr_ocupare = _.ocupare FROM sync _ WHERE _.circumscriptie = s.nr_circumscriptie::smallint AND _.sectie = s.numar_sectie::smallint'
      ];
