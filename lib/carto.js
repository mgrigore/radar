...
module.exports = {
  ...
  refresh: function (date) {
    ...
    function primary() {
      let queries = [
        'UPDATE sectii_tara s SET last_update = \'' + date + '\', nr_votanti = _.voturi, pr_votanti = ROUND(COALESCE(100::real * _.voturi / s.votanti_estimati::integer, 0)::numeric, 1) FROM sync _ WHERE _.circumscriptie = s.nce_judet::smallint AND _.sectie = s.numar_sectie::smallint',
        'UPDATE sectii_diaspora s SET last_update = \'' + date + '\', nr_votanti = _.voturi, pr_votanti = ROUND(COALESCE(100::real * _.voturi / NULLIF(s.votanti_estimati::integer, 0), 100::real * _.voturi / NULLIF(_.voturi, 0),  0)::numeric, 1), gr_ocupare = _.ocupare FROM sync _ WHERE _.circumscriptie = s.nr_circumscriptie::smallint AND _.sectie = s.numar_sectie::smallint'
      ];
    ...
    function secondary() {
      let queries = [
        'UPDATE tari t SET last_update = _.last, nr_votanti = _.voturi, pr_votanti = ROUND(COALESCE(100::real * _.voturi / NULLIF(t.nr_estimat::integer, 0), 100::real * _.voturi / NULLIF(_.voturi, 0), 0)::numeric, 1) FROM (SELECT s.cod_tara, MAX(s.last_update), SUM(s.nr_votanti) FROM sectii_diaspora s GROUP BY 1) AS _(iso, last, voturi) WHERE _.iso = t.iso_code',
        'UPDATE uat u SET last_update = _.last, nr_votanti = _.voturi, pr_votanti = ROUND(COALESCE(100::real * _.voturi / NULLIF(u.votanti_estimati::integer, 0), 100::real * _.voturi / NULLIF(_.voturi, 0), 0)::numeric, 1) FROM (SELECT s.siruta_uat, MAX(s.last_update), SUM(s.nr_votanti) FROM sectii_tara s GROUP BY 1) AS _(siruta, last, voturi) WHERE _.siruta = u.siruta',
        'UPDATE judet j SET last_update = _.last, nr_votanti = _.voturi, pr_votanti = ROUND(COALESCE(100::real * _.voturi / NULLIF(j.votanti_estimati::integer, 0), 100::real * _.voturi / NULLIF(_.voturi, 0), 0)::numeric, 1) FROM (SELECT s.siruta_judet::integer, MAX(s.last_update), SUM(s.nr_votanti) FROM sectii_tara s GROUP BY 1) AS _(siruta, last, voturi) WHERE _.siruta = j.siruta_judet'
      ];
